...
using WebApi.Conversion4.Models.Provider.MasterData;
using WebApi.Conversion4.Services.CustomerA;
...
{
    public class CustomerASupplierPsTool : PsTool 
    {
        public override Map CreateMap()
        {
            return new CustomerASupplierMap();
        }

        public override Document CreateNewDocument()
        {
            return new MasterDataDocument(new SupplierDocumentHeader());
        }
        ...
        public override StringBuilder ConvertDocumentsToStandardFile()
        {
            ...
            try
            {
                var groupedDocuments = Documents.GroupBy(s => (s.Header as SupplierDocumentHeader).VendorId);
                var preVendor = new SupplierDocumentHeader();
                foreach (var documents in groupedDocuments)
                {
                    int i = 0;
                    foreach (var doc in documents)
                    {
                        var header = (doc.Header as SupplierDocumentHeader);
    ...
    public class MasterDataDocument : Document
    {
        ...
        {
            return new SupplierDocumentDetailLine();
        }
